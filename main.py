...
# ---------------------------------------------------------------------------
# OAuth 2.1 / MCP HTTP authorization configuration
# ---------------------------------------------------------------------------
...
def _oauth_http_enabled() -> bool:
    return _env_bool("MCP_OAUTH_ENABLED", True)


def _normalize_mcp_path() -> str:
...
def _oauth_resource_url(request: Any) -> str:
    return os.getenv(
        "MCP_OAUTH_RESOURCE",
        f"{_request_origin(request)}{_normalize_mcp_path()}",
    ).rstrip("/")
...
def _validate_authorization_params(params: dict[str, str], request: Any) -> tuple[
    Optional[dict[str, Any]], Optional[list[str]], Optional[Any]
]:
...
class OAuthProtectedASGI:
    def __init__(self, app: Any) -> None:
        self.app = app
        self.mcp_path = _normalize_mcp_path()

    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        if scope["type"] != "http" or not _oauth_http_enabled():
...
async def _run_oauth_http_server() -> None:
    import uvicorn
...
async def _main() -> None:
    try:
        ...
        if transport == "http":
            if _oauth_http_enabled():
                await _run_oauth_http_server()
            else:
